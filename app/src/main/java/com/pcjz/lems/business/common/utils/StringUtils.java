...
public class StringUtils {
	...
	public static String bitmapToString(Bitmap bitmap) {
		...
		if (BitmapUtils.getBitmapSize(bitmap) > 1024*200) {
			bitmap = BitmapUtils.compressImage(bitmap);
			//bitmap.compress(CompressFormat.JPEG, AppConfig.BITMAP_COMPRESS_PERCENT, bStream);
		}
		bitmap.compress(CompressFormat.JPEG, 100, bStream);
		byte[] bytes = bStream.toByteArray();
		String strBase64 = Base64.encodeToString(bytes, 0, bytes.length,
				Base64.NO_WRAP);// (bytes,Base64.DEFAULT);
		return strBase64;
	}
